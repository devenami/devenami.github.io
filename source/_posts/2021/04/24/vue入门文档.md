...
声明式渲染是`vue`提供的一个动态改变页面属性的特性，通过页面属性和`js`属性的绑定，实现修改一方，另一方也动态更新的原则。
...
`vue`只允许访问一个全局变量，同时，`js`表达式只能是一行，不支持多行。



## 指令

指令是带有`v-`前缀的特殊特性。指令的值预期是**单个js表达式**
...
`vue`为`v-bind`和`v-on`这两个常用指令提供了缩写。
...
计算属性用于简化`{{}}`中频繁的使用复杂的表达式。计算属性和普通属性的使用方式一致。当`Vue`对象的`data`在计算属性内使用时，如果`data`内的数据发生变更，计算属性也会跟随着变化。
...
`计算属性`和`方法`都能达到属性计算的目的，但是`计算属性`是惰性求值的，也就是说，`计算属性`会做缓存，只有与`计算属性`关联的属性改变时，`计算属性`才会重新求值。而方法则是在每次调用时都会重新求值。
...
Vue也提供了`v-else-if`，这些判断表达式都可以引用在`template`上面。

**注： **Vue会重用已有的元素而不是从头开始渲染，如果要禁止该特性，可以给标签添加`key`属性，提供一个唯一值。
...
`v-for`用于对列表进行渲染，`v-for` 指令需要使用 `item in items` 形式的特殊语法，`items` 是源数据数组并且 `item` 是数组元素迭代的别名。`v-for` 还支持一个可选的第二个参数为当前项的索引。
...
`v-for`应用在对象上，格式为`field in object`， `field`为对象的属性值。在该语法中，field可以提供三个参数，分别是 （属性值， 属性名， 索引）
...
`v-model`会忽略所有表单元素的`value`、`checked`、`selected`特性的初始值而总是将Vue实例的数据作为数据来源。因此，应该通过JS在组件的`data`选项中声明初始值。
...
**注： ** `v-model` 和 `v-bind:value` 绑定的值一致。
...
子组件可以通过`props`选项将其包含在该组件可接受的prop列表中。
...
我们可以使用`v-bind`来动态传递prop。也可通过`v-bind`直接传递对象。
...
子组件通过`v-on:事件类型="$emit('事件名')"`向父组件提交事件，父组件通过`v-on:事件名`绑定并处理事件。
...
自定义组件需要使用`$emit`提交事件，并提供组件的输入值。同时也需要为自定义组件绑定值。
...
使用自定义组件和使用原生组件一致。`v-model`既可以绑定`data`的属性，也可以绑定计算属性，但是不能绑定方法。
